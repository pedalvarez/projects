	...
	public partial class SqlConnectionUIControl : UserControl, IDataConnectionUIControl
	{
        ...
        {           
			...
			try
			{
                ...
                string d = connection.Database;

                connection.ChangeDatabase("master");

                IDbCommand command = connection.CreateCommand();
                command.CommandText = "SELECT * FROM sysdatabases where name = '" + selectDatabaseComboBox.Text + "'";
                ...
                if (reader.Read() )
                {
                    DialogResult ret = MessageBox.Show("The database " + selectDatabaseComboBox.Text + " already exists. Do you want to drop it and create a new one with that name", "MIP Toolkit Configurator", MessageBoxButtons.YesNo);
                    ...
                    command.CommandText = "DROP DATABASE " + selectDatabaseComboBox.Text; 

                    command.ExecuteNonQuery();

                    command.CommandText = "SELECT * FROM master.dbo.sysdatabases where name = '" + selectDatabaseComboBox.Text + "'";

                    reader = command.ExecuteReader();

                    if (reader.Read())
                    {
                        MessageBox.Show("Failed to drop database " + selectDatabaseComboBox.Text, "MIP Toolkit Configurator");
                        return;                    
                    }
                }
                ...
                command.CommandText = "CREATE DATABASE " + selectDatabaseComboBox.Text; 

                command.ExecuteNonQuery();

                MessageBox.Show("Successfully created the database " + selectDatabaseComboBox.Text, "MIP Toolkit Configurator");
			}
			catch(SqlException ex)
			{
                MessageBox.Show("Failed to create database " + selectDatabaseComboBox.Text + ". " + ex.Message, "MIP Toolkit Configurator");
			}
			finally
			{
                if (reader != null)
