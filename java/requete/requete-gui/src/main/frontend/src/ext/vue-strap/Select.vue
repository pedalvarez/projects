  ...
  <div ref="select" :class="classes" v-click-outside="close">
    ...
    <ul class="dropdown-menu">
      <template v-if="list.length">
        ...
        <li v-if="required&&!clearButton"><a @mousedown.prevent="clear() && close()">{{ placeholder || text.notSelected
          }}</a></li>
    ...
    computed: {
      ...
      classes () {
        return [{open: this.show, disabled: this.disabled}, this.class,
                                                            this.isLi ? 'dropdown' : this.inInput ? 'input-group-btn'
                                                              : 'btn-group']
      },
      filteredOptions () {
      ...
      selected () {
        ...
        var sel = this.values.map(
          val => (this.list.find(o => o[this.optionsValue] === val) || {})[this.optionsLabel]).filter(
          val => val !== undefined)
